...
class SpecificPartyCubit extends Cubit<SpecificPartyState> {
  ...
  ///get all active memberships by party id
  void getAllActiveMembership(String id) async {
    emit(SpecificPartyLoading());
    final activeMemberships = await _partyService.getActiveMembership(id);

    emit(SpecificPartyListRender(activeMemberships: activeMemberships));
  }
